...
#SBATCH --job-name=mapping_star        # 作业名
...
# 处理一下 zip 文件，star 不支持输入 zip 文件   
if [[ $input_forward == *gz ]] ; then
  echo $input_forward "is gzip file" ;
  gunzip $input_forward 
  input_forward=${input_forward%%\.gz}
fi

if [[ $input_reverse == *gz ]] ; then
  echo $input_reverse "is gzip file" ;
  gunzip $input_reverse 
...
STAR --outFilterType BySJout --runThreadN $thread --outFilterMismatchNmax 2 --genomeDir $reference_genome  \
  --readFilesIn $input_forward  $input_reverse --outFileNamePrefix ../mapping/${sample_name}  --outSAMtype BAM \
